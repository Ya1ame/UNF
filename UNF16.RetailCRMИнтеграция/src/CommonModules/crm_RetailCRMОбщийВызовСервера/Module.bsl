...
Процедура ВыгрузитьExternalIdsТоваров(Док) Экспорт
	...
	Для каждого стр Из Док["Запасы"] Цикл
		...
		XmlID = ИдентификаторТовара(стр, ИдентификаторТовара);
		...
		Если ЗначениеЗаполнено(НайдЭлемент) Тогда
			...
			ИдентифПоз.Вставить("value",	Строка(стр.КлючСвязи));
			...
			Для каждого Элемент Из НайдЭлемент Цикл
				ИдентифПоз = Новый Соответствие();
				ИдентифПоз.Вставить("code",		Строка(Элемент["Ключ"]));		
				ИдентифПоз.Вставить("value",	Строка(Элемент["Значение"]));
				ИдентификаторыПозиции.Добавить(ИдентифПоз);
...
Процедура ВыгрузитьЧекВCRM(Док, Отменить = Ложь, ДанныеПЛ = Неопределено) Экспорт
	...
	Если СтрНайти(Ответ, """success"":true") > 0 Тогда
		...
		Если ЗначениеЗаполнено(ИДЗаказа) И ЗначениеЗаполнено(Док.crm_idКлиента) И Док.БонусныеБаллыПЛ > 0 Тогда 
...
Функция СписатьБаллыВCRM(Док, ИДЗаказа) Экспорт
	...
	Переменные.Вставить("apiKey", ApiKey);
	Переменные.Вставить("site", КодМагазина);
	Переменные.Вставить("bonuses", Формат(Число(Док.БонусныеБаллыПЛ), "ЧГ=0"));
	Заказ = Новый Соответствие();
...
Функция ИдентификаторТовара(СтруктураНоменклатуры, ИдентификаторТовара) Экспорт
