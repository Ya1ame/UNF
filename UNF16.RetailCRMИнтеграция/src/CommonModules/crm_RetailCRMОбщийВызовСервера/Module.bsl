...
Функция HTTPзапросPOST(АдресСервера, АдресСкрипта, НаборПеременных) Экспорт
	...
	СтрокаСоединения = Строка(СтрокаСоединенияИнформационнойБазы());
...
Функция ПреобразоватьПолученнуюДату(Дата) Экспорт
...
Процедура ВыгрузитьЧекВCRM(Док, Отменить = Ложь, ДанныеПЛ = Неопределено) Экспорт
	...
	Если ПолучитьЗначениеКонстанты("НесколькоОрганизаций") = Истина Тогда
		...
		НайденнаяСтрока = СписокОрганизаций.Найти(Док.Организация, "Организация"); 
	...
	Если ЗначениеЗаполнено(Док.Ответственный) Тогда  
		СписокМенеджеров = ПолучитьЗначениеКонстанты("ТабМенеджеры");
		НайденнаяСтрока = СписокМенеджеров.Найти(Док.Ответственный, "Менеджер"); 
	...
	Для каждого Оплата Из Док.БезналичнаяОплата Цикл
		Если Оплата.ВидОплаты = Перечисления.ВидыБезналичныхОплат.БанковскаяКарта Тогда
			...
			СписокТиповОплат = ПолучитьЗначениеКонстанты("ТабТипыОплат");
			ТипыОплат		 = СписокТиповОплат.Найти(Оплата.ВидОплаты, "ТипОплатыРозница");
	...
	Если Док.ПолученоНаличными <> 0 Тогда 
		...
		СписокТиповОплат = ПолучитьЗначениеКонстанты("ТабТипыОплат");
		ТипыОплат		 = СписокТиповОплат.Найти(Истина, "НаличнаяОплатаРозница");
	    Если ТипыОплат <> Неопределено Тогда
			КодТипаОплаты = ТипыОплат.КодCRM;                                         
	...
	Если СтрНайти(Ответ, """success"":true") > 0 Тогда
		...
		Если ЗначениеЗаполнено(ИДЗаказа) И ЗначениеЗаполнено(Док.crm_idКлиента) И Док.crm_БонусныеБаллыПЛ > 0 Тогда 
			Ответ = СписатьБаллыКлиента(Док, ИДЗаказа);                                                
		КонецЕсли;
...
Процедура ВыгрузитьЧекВозвратВCRM(ДокВозврат) Экспорт
	...
	Если ПолучитьЗначениеКонстанты("НесколькоОрганизаций") = Истина Тогда
			СписокОрганизаций = ПолучитьЗначениеКонстанты("СоответствиеОрганизаций");
			НайденнаяСтрока = СписокОрганизаций.Найти(ДокументЧекККМ.Организация, "Организация"); 
			Если НайденнаяСтрока <> Неопределено Тогда
				КодМагазина = НайденнаяСтрока.КодМагазина;                                         
			КонецЕсли;
	КонецЕсли;
...
Функция Из_10(Значение = 0, Нотация = 36)
...
Функция ПолучитьЗначениеИзЗапроса(СтрокаДанных, Значение, Позиция = 1, Ключ = "")
	...
	Пока Позиция <= СтрДлина(СтрокаДанных) Цикл
		Элемент1 = Сред(СтрокаДанных, Позиция, 2);
		...
		Если Элемент2 > " " ИЛИ Строка Тогда // Отсекаем всякий хлам
			...
			ИначеЕсли Элемент2 = "\" Тогда
				...
				ИначеЕсли Элемент1 = "\U" Тогда
					ЗначениеВСтроке = ЗначениеВСтроке + Символ(Hex2Число(Сред(СтрокаДанных, Позиция, 4))); // шестнадцатиричное число
					Позиция = Позиция + 4;
				КонецЕсли;
			...
			ИначеЕсли СтрНайти("""'{}[]:,", Элемент2) > 0 Тогда
				...
				ИначеЕсли Элемент2 = "{" Тогда
					...
					Пока Истина Цикл
						...
						Ключ = "";
						Режим = ПолучитьЗначениеИзЗапроса(СтрокаДанных, Значение, Позиция, Ключ);
				...
				ИначеЕсли Элемент2 = "[" Тогда
					...
					Массив = Новый Массив;
					Пока Истина Цикл
						Режим = ПолучитьЗначениеИзЗапроса(СтрокаДанных, Значение, Позиция);
				...
				ИначеЕсли Элемент2 = ":" Тогда
					...
					Возврат ПолучитьЗначениеИзЗапроса(СтрокаДанных, Значение, Позиция);
...
Функция СписатьБаллыКлиента(Док, ИДЗаказа)
...
Функция ВыделитьСлово(ИсходнаяСтрока, Позиция) 
	...
	КонечнаяПозицияСтроки = 3;
	
	Если Позиция = КонечнаяПозицияСтроки Тогда 
...
Функция ПолучитьПаки(НомерЗаказа)
	
	ИмяСервера = ПолучитьЗначениеКонстанты("ИмяСервера");
	ApiKey = ПолучитьЗначениеКонстанты("КлючCRM");
	Ответ = HTTPзапросGET(ИмяСервера, 
	...
	Если Успех Тогда
		Для каждого пак Из Ответ["packs"] Цикл
				...
				ИначеЕсли ИдентификаторТовара = "GUID" Тогда
					UUID 	= Новый УникальныйИдентификатор(пак["item"]["offer"]["xmlId"]);
