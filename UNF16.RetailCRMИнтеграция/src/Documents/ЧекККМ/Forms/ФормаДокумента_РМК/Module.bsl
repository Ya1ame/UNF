...
Процедура crm_ЗаменитьКлиента(ДанныеПЛ) Экспорт 
	...
	Контрагент = Обработки.crm_RetailCRMОбработка.УНФ_ВернутьКонтрагентаПоID(Формат(Число(ДанныеПЛ.IDКлиента), "ЧГ=0"));
	...
	Иначе
		...
		Обработки.crm_RetailCRMОбработка.УНФ_УстановитьСвойствоУОбъекта(Контрагент.Ссылка
														  , СвойствоКлиента, Формат(Число(ДанныеПЛ.IDКлиента), "ЧГ=0"));  
		//
...
Процедура crm_РассчитатьСкидкуПоПЛНаСервере(ДанныеПЛ) Экспорт 
	...
	Если СтрНайти(Ответ, """success"":true") > 0 Тогда
		...
		Для Каждого СтрТоваров Из Объект["Запасы"] Цикл 
			...
			СтруктураНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрТоваров.Номенклатура
																		  , "ИспользоватьХарактеристики, Код, Артикул");			
			Если crm_RetailCRMОбщийВызовСервера.ПолучитьЗначениеКонстанты("ИспользованиеХарактеристик") = Истина и 
		                                           СтруктураНоменклатуры.ИспользоватьХарактеристики = Истина Тогда

				XmlID = СокрЛП(СтрТоваров.Номенклатура.УникальныйИдентификатор());
				XmlID = ?(ЗначениеЗаполнено(СтрТоваров.Характеристика), 
				XmlID + "#" + СокрЛП(СтрТоваров.Характеристика.УникальныйИдентификатор()), XmlID);
				
				СтрТЗ.Идентификатор = XmlID; 
		
			Иначе 
				
				Если ИдентифТовара = "Код" Тогда
					СтрТЗ.Идентификатор = СокрЛП(СтруктураНоменклатуры.Код);
				ИначеЕсли ИдентифТовара = "Артикул" Тогда
					СтрТЗ.Идентификатор	= СокрЛП(СтруктураНоменклатуры.Артикул);
				ИначеЕсли ИдентифТовара = "GUID" Тогда
					СтрТЗ.Идентификатор	= СокрЛП(СтрТоваров.Номенклатура.УникальныйИдентификатор());	
				КонецЕсли;
		...
		Для Каждого стрТовар Из Ответ["order"]["items"] Цикл
			...
			Если ИскСтрокиТЗ <> Неопределено Тогда 
				Для Каждого ИскСтрТЗ Из ИскСтрокиТЗ Цикл
					...
					Если ИскСтрокиТЧ <> Неопределено Тогда 
		...
		Объект.БонусныеБаллыПЛ = КоличествоБонусныхБаллов;    
		БонуснаяКарта = crm_RetailCRMОбщийВызовСервера.ПолучитьЗначениеКонстанты("БонуснаяКарта");
		Если Объект.БонусныеБаллыПЛ <> 0 Тогда
...
Процедура crm_СоздатьДокументНачислениеИСписаниеБаллов(БонуснаяКарта,Партнер,НачислитьБонусы)
	...
	Если ОстатокКартыЛояльности.КСписанию < 0 Тогда    
