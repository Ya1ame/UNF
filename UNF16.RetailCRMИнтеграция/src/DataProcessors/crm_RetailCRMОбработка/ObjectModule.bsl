...
Процедура Мод_ЗагрузитьЗаказы(ИспДатыАктуальности = Неопределено, ДатаИзменений = Неопределено, ИДИзменений = Неопределено) Экспорт
	...
	Иначе // загрузка по SinceId
		...
		Если ИДИзменений = 0 Тогда 
			ОбщегоНазначения.СообщитьПользователю("Не указан id последних изменений."
									   + " Укажите id ИЛИ загрузите по дате актуальности, id заполнится автоматически");
			Возврат;
		Иначе
			...
			ЗаписьЖурналаРегистрации("RetailCRM"
								...
								,
								,
								, "Загрузка заказов идентификатору изменений: " + ИДИзменений);
...
Процедура Мод_ВыгрузкаКаталогаВXML(ТабКаталога, ПутьКфайлу) Экспорт
	...
	ФильтрВыгрузки = ТабКаталога.Выгрузить(); 
	ФильтрВыгрузки = ?(ЗначениеЗаполнено(ФильтрВыгрузки), ФильтрВыгрузки, "");
	
	Рез = УНФ_ЗапросДляКаталога(ФильтрВыгрузки);
	...
	Если ЗначениеЗаполнено(ФильтрВыгрузки) Тогда 
	...
	Запрос.УстановитьПараметр("Родитель", ФильтрВыгрузки);
	...
	Если ЗначениеЗаполнено(ФильтрВыгрузки) Тогда 
		Для Каждого стрГруппа Из ФильтрВыгрузки Цикл 
...
Процедура Мод_ВыгрузкаКаталогаВXMLХарактеристики(ТабКаталога, ПутьКФайлу) Экспорт
	...
	Рез = УНФ_ЗапросДляКаталогаХарактеристики(ФильтрВыгрузки);
...
Процедура Мод_ОстаткиИЦеныВCRM() Экспорт 
	...
	Для каждого Стр Из РезультатЗапроса Цикл    		
		...
		//  Код склада
		СимвольныйКодСклада = "";
		НайденнаяСтрока = СкладыДляОбмена.СтрНайти(Стр.СтруктурнаяЕдиница);
		...
		Склад = Новый Соответствие();
		Склад.Вставить("code", СимвольныйКодСклада);
		Склад.Вставить("available", СтрЗаменить(строка(Остаток), Символ(160), ""));
		Склад.Вставить("purchasePrice", СтрЗаменить(строка(Цена), Символ(160), "")); 
...
Процедура Мод_ОстаткиИЦеныВCRMХарактеристики() Экспорт 
	...
	РезультатЗапроса = ПолучитьТаблицуОстатковСХарактеристиками(Склады, Родитель);
...
Процедура РегистрацияСкладскойСистемыНаСервере() Экспорт
	...
	Переменные.Вставить("integrationModule", crm_RetailCRMОбщийВызовСервера.JSONTest(Конфигурация));
...
Функция ПолучитьТаблицуОстатковСХарактеристиками(Склады, Родитель)
...
Процедура ЗагрузитьСтатусы(ТабСтатусы) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИмяСервера) ИЛИ Не ЗначениеЗаполнено(ApiKey) Тогда 
	...
	Статусы = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера
																  , "/api/v5/reference/statuses?apiKey=" + ApiKey);
	...
	ГруппыСтатусов = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера
															 , "/api/v5/reference/status-groups?apiKey=" + ApiKey);
...
Процедура ЗагрузитьСтатусыТоваров(ТабСтатусыТоваров) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСервера) ИЛИ ЗначениеЗаполнено(ApiKey) Тогда
		
		СтатусыТоваров = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера,
																"/api/v5/reference/product-statuses?apiKey=" + ApiKey);
...
Процедура ЗагрузитьМенеджеров(ТабМенеджеров) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСервера) ИЛИ ЗначениеЗаполнено(ApiKey) Тогда
		...
		Менеджеры = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера, "/api/v5/users?apiKey=" + ApiKey);
...
Процедура ЗагрузитьСклады(ТабСклады) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСервера) ИЛИ ЗначениеЗаполнено(ApiKey) Тогда
		...
		Склады = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера, "/api/v5/reference/stores?apiKey=" + ApiKey);
...
Процедура ЗагрузитьТипыЦен(ТабТиповЦен) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСервера) ИЛИ ЗначениеЗаполнено(ApiKey) Тогда
		...
		ТипыЦен = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера
															  , "/api/v5/reference/price-types?apiKey=" + ApiKey);
...
Процедура ЗагрузитьТипыОплат(ТабТиповОплат) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСервера) ИЛИ ЗначениеЗаполнено(ApiKey) Тогда
		...
		ТипыОплат = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера
																, "/api/v5/reference/payment-types?apiKey=" + ApiKey);
...
Процедура ЗагрузитьТипыДоставок(ТабДоставок) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСервера) ИЛИ	ЗначениеЗаполнено(ApiKey) Тогда
		...
		ТипыДоставок = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера
																, "/api/v5/reference/delivery-types?apiKey=" + ApiKey);
...
Процедура ЗагрузитьСпособыОформления(ТабСпособыОформления) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяСервера) ИЛИ ЗначениеЗаполнено(ApiKey) Тогда
		...
		СпособыОформления = crm_RetailCRMОбщийВызовСервера.HTTPзапросGET(ИмяСервера, 
																   "/api/v5/reference/order-methods?apiKey=" + ApiKey);
...
Процедура Мод_ЗагрузитьБрониПоЗаказу(ИДЗаказа, ТЗБрони)
...
Функция ПолучитьИсториюИзменений(Фильтр = "", ИспДатыАктуальности = Ложь) 
	...
	Если ИспДатыАктуальности = Истина Тогда
		...
		ДатаИзСРМ = crm_RetailCRMОбщийВызовСервера.ДатаВремяДляCRM(Фильтр); 
		ШаблонСкрипта = "/api/v5/orders/history?filter[startDate]=%1&apiKey=%2&page=%3";
		АдресСкрипта = СтрШаблон(ШаблонСкрипта, ДатаИзСРМ, ApiKey, 1);
		
	Иначе 
		
		ШаблонСкрипта = "/api/v5/orders/history?filter[sinceId]=%1&apiKey=%2";
		АдресСкрипта = СтрШаблон(ШаблонСкрипта, Формат(Фильтр, "ЧГ=0"), ApiKey);
	...
	ТЗЗаказы.Колонки.Добавить("НомерЗаказа", Новый ОписаниеТипов("Строка"));
	
	Для каждого Стр Из Ответ["history"] Цикл
		
		Если Стр["source"] = "api" Тогда 
			Если Стр["apiKey"] <> Неопределено И Стр["apiKey"]["current"] = Истина Тогда 
		...
		СтрТЗ.НомерЗаказа = Формат(Стр["order"]["id"], "ЧГ=0");
		ПоследнееИзменение = Стр["id"]; 
	...
	Если Ответ["pagination"] <> Неопределено Тогда
		...
		Для ин = 2 По ВсегоСтраниц Цикл
			...
			Если ИспДатыАктуальности = Истина Тогда
				АдресСкрипта = СтрШаблон(ШаблонСкрипта, ДатаИзСРМ, Строка(Формат(ин, "ЧГ=0")), ApiKey);
			Иначе
				АдресСкрипта = СтрШаблон(ШаблонСкрипта, Формат(ПоследнееИзменение, "ЧГ=0"), ApiKey);
			КонецЕсли;				
			
			Ответ = ЗапросCRM(АдресСкрипта, ИмяСервера);
			
			Для каждого Стр Из Ответ["history"] Цикл
				Если Стр["source"] = "api" И Не Стр["apiKey"] <> Неопределено Тогда 
...
//	ТЗЗаказы = НайтиДокументыТаблицы(ТЗЗаказы);
...
Функция ФормированиеСтруктурыЗаказа(Стр)
	...
	Если ДатаСоздания <> Неопределено Тогда 
		ДатаСоздания =  crm_RetailCRMОбщийВызовСервера.ПреобразоватьПолученнуюДату(ДатаСоздания);
	Иначе
	...
	Если ДатаДоставки <> Неопределено Тогда 
		ДатаДоставки =  crm_RetailCRMОбщийВызовСервера.ПреобразоватьПолученнуюДату(ДатаДоставки);
	Иначе
	...
	ВремяДоставкиС  = crm_RetailCRMОбщийВызовСервера.ПреобразоватьПолученнуюДату(ВремяДоставкиС);
	ВремяДоставкиПо = crm_RetailCRMОбщийВызовСервера.ПреобразоватьПолученнуюДату(ВремяДоставкиПо);
	...
	Если ДатаОтгрузкиПлан <> Неопределено Тогда 
		ДатаОтгрузкиПлан =  crm_RetailCRMОбщийВызовСервера.ПреобразоватьПолученнуюДату(ДатаОтгрузкиПлан);
	Иначе
	...
	Для Каждого стрОпл Из Стр["payments"] Цикл 
		...
		стрОплата.Вставить("ДатаОплаты",	?(стрОпл.Значение["paidAt"] <> Неопределено, 
																  crm_RetailCRMОбщийВызовСервера.ПреобразоватьПолученнуюДату(стрОпл.Значение["paidAt"]), ""));
...
Процедура Мод_СозданиеЗаказов(МасивЗаказов, ТЗброни = Неопределено)
	...
	Для каждого Стр Из МасивЗаказов Цикл
		...
		Если Стр["items"].Количество() = 0 Тогда
			ОбщегоНазначения.СообщитьПользователю("Заказ № " 
												 + Стр["number"] 
												 + " не был загружен в 1С, т.к в нем отсутствуют товары!");
			Продолжить;
		КонецЕсли;
		...
		Если НаборСтатусов <> Неопределено Тогда
			Если НаборСтатусов.Количество() > 0 Тогда
				...
				Если ВыбранныйСтатус = Неопределено ИЛИ ВыбранныйСтатус.НеЗагружать Тогда 
					ОбщегоНазначения.СообщитьПользователю("Заказы в статусе " 
									+ Стр["status"] 
									+ " не загружаются в 1С. Чтобы это исправить, перейдите "
									+ " на вкладку ""Статусы"" И добавьте этот статус в таблицу "
									+ " или отключите галочку не загружать.");
					Продолжить;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Если ИспользоватьСпособыОформленияЗаказов <> Неопределено И ИспользоватьСпособыОформленияЗаказов = Истина Тогда
			Если ОграничениеПоСпособуОформления <> Неопределено И ОграничениеПоСпособуОформления = Истина Тогда
				...
				Если НаборСпособов <> Неопределено И НаборСпособов.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(ИскДок) Тогда                      
			УНФ_СоздатьЗаказ(стрЗаказа, Стр, ИскДок, ВыбранныйСтатус, ТЗброни);
		Иначе                                                     
			УНФ_СоздатьЗаказ(стрЗаказа, Стр, , ВыбранныйСтатус, ТЗброни);
		КонецЕсли;  
...
Процедура УНФ_СоздатьЗаказ(ДанныеЗаказа, Стр, ИскДок, СтатусДействия, ТЗброни = Неопределено)
	...
	Если ЗначениеЗаполнено(ДанныеЗаказа.КодМагазина) Тогда 
		Если crm_RetailCRMОбщийВызовСервера.ПолучитьЗначениеКонстанты("НесколькоОрганизаций") = Истина Тогда 
			НесколькоОрганизаций = Истина;
			ЭлементОрганизация = Справочники.crm_Организации.НайтиПоРеквизиту("КодМагазинаCRM", ДанныеЗаказа.КодМагазина);
			Организация = ЭлементОрганизация.Организация;                                         
		КонецЕсли;
	КонецЕсли;
	...
	Если не ЗначениеЗаполнено(ИскДок) Тогда
		...
		Док.ВалютаДокумента 		= 	?( ЗначениеЗаполнено(Док.ВалютаДокумента)
										, Док.ВалютаДокумента
										, crm_RetailCRMОбщийВызовСервера.ПолучитьЗначениеКонстанты("Валюта"));
	...
	Иначе
		...
		Если РеализацияПриВозврате <> Неопределено И РеализацияПриВозврате.Проведен Тогда
			...
			Если ДокВозврата <> Документы.ПриходнаяНакладная.ПустаяСсылка() Тогда
				ОбщегоНазначения.СообщитьПользователю("По документу " 
													 +  Док.Ссылка 
													 + " создан документ возврата, обновление невозможно");
				Возврат;
			Иначе
	...
	Если Не ЗначениеЗаполнено(Склад) Тогда 
		Если НесколькоОрганизаций = Истина Тогда
			Склад = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементОрганизация, "Склад");
		Иначе 
	...
	Если СтатусДействия.Отгрузка Тогда 
		...
		Документ = УНФ_ВернутьДокументПеремещения(Док.Ссылка, СтруктурнаяЕдиницаРезерв); 
	...
	Если ДанныеЗаказа["КаналПривлечения"] <> Неопределено Тогда
		ВыбраннаяСтрока = Справочники.ИсточникиПривлеченияПокупателей.НайтиПоРеквизиту("crm_КодRetail"
																					, ДанныеЗаказа["КаналПривлечения"]);
	...
	Если СтатусДействия.Производство Тогда 
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю("Таблица статусов товаров не заполнена." + Символы.ПС 
					+ "Заказ " + ДанныеЗаказа["НомерЗаказа"] + " был загружен. Дальнейшие действия приостановлены!" 
	...
	Если СтатусДействия.Отгрузка И ОтменаЗаказа = Ложь Тогда
		Если НесколькоОрганизаций = Истина Тогда
			ОрганизацияРеализация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЭлементОрганизация, "Реализация");
	...
	Если СтатусДействия.Резервирование Тогда
		...
		Если СтатусыТоваров <> Неопределено И типзнч(СтатусыТоваров) = тип("ТаблицаЗначений") Тогда
			...
			Если СтрокиСтатусаКПроизводству.Количество() <> 0 Тогда
				...
				ТоварыВРезерве = Док["Запасы"].НайтиСтроки(Новый Структура("crm_Статус", "in-reserve"));
				Если ТоварыВРезерве.Количество() = Док["Запасы"].Количество() Тогда
...
Процедура УНФ_СоздатьДокументОплаты(Док, Оплата, ТипДокументаОплаты, ДокументОплаты)
	...
	Если ТипДокументаОплаты = "Постуление безналичных денежных средств" Тогда 
		...
		СвойствоОплаты = Обработки.crm_RetailCRMОбработка.УНФ_ПолучитьСвойство("ИДоплаты"
									  , Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_ПоступлениеНаСчет);
	...
	ИначеЕсли ТипДокументаОплаты = "Поступление наличных денежных средств" Тогда
		...
		СвойствоОплаты = Обработки.crm_RetailCRMОбработка.УНФ_ПолучитьСвойство("ИДоплаты"
									  , Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_ПоступлениеВКассу);
	...
	ЗаписатьДокумент(ДокОБ);
		
	Если ДокОБ <> Неопределено Тогда
		Обработки.crm_RetailCRMОбработка.УНФ_УстановитьСвойствоУОбъекта(ДокОБ.Ссылка
																		, СвойствоОплаты
																		, Формат(Оплата["ИДОплаты"], "ЧГ=0"));
	КонецЕсли;
...
Функция УНФ_СоздатьРеализацию(Док, Реализация)
	...
	Попытка
		...
		ОбщегоНазначения.СообщитьПользователю("Документ проведен: " + ДокументРеализации);
		Возврат Истина;
		
	Исключение
		ОбщегоНазначения.СообщитьПользователю("Не удалось провести документ: " + ДокументРеализации + " " + ОписаниеОшибки());
		Попытка
			ДокументРеализации.Записать(РежимЗаписиДокумента.Запись);
			ОбщегоНазначения.СообщитьПользователю("Документ записан: " + ДокументРеализации);
		Исключение
			ОбщегоНазначения.СообщитьПользователю("Не удалось записать документ: " + ДокументРеализации + " " + ОписаниеОшибки());
		КонецПопытки;
		Возврат Ложь;
...
Процедура УНФ_СоздатьСтрокуДокумента(Док, Стр, ОтменаЗаказа = Ложь, Эталонная, ТекСтр, СоответствиеСтатусов)
	...
	Иначе 
		...
		ИначеЕсли ИдентификаторТовара = "GUID" Тогда
			UUID 	= Новый УникальныйИдентификатор(XmlID);
			ИскНом	= Справочники.Номенклатура.ПолучитьСсылку(UUID);	
		КонецЕсли;
	...
	Если Стр["СтоимостьДоставки"] <> Неопределено  Тогда
		...
		Если ЗначениеЗаполнено(Стр["СтоимостьДоставки"]) И Стр["СтоимостьДоставки"] <> "0" И Не ОтменаЗаказа Тогда
				...
				Если ТипыДоставок = Неопределено Тогда 
					ОбщегоНазначения.СообщитьПользователю("Не заполнена таблица соответствий типов доставки." + Символы.ПС 
...
Процедура УНФ_СоздатьЗаказНаПроизводство(Док, Стр, СтатусДействия,ТоварыКПроизводству)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТехоперации") 
			И Константы.АвтоматическиПланироватьОперацийЗаказомНаПроизводство.Получить() Тогда
...
Функция УНФ_ОпределениеКонтрагента(Стр, Организация)
	...
	ИскКонт = crm_RetailCRMОбщийВызовСервера.УНФ_ВернутьКонтрагентаПоID(IdКонтрагента); 
...
Функция УНФ_ВернутьДокументПеремещения(Док, СкладОтправитель)
...
Функция УНФ_ВернутьРеализацию(Заказ)
	...
	ИскРеализация = Неопределено;
	Если Выборка.Следующий() Тогда
		ИскРеализация = Выборка;
	КонецЕсли;
...
Функция УНФ_ВернутьНоменклатуру(XmlID)
	...
	Если Инд = 0 Тогда
		Попытка
			...
			UUID 				= Новый УникальныйИдентификатор(XmlID);
			ИскНоменклатура		= Справочники.Номенклатура.ПолучитьСсылку(UUID);
	...
	Иначе 
		...
		Попытка
			UUID 				= Новый УникальныйИдентификатор(МассивЗначений[0]);
			ИскНоменклатура		= Справочники.Номенклатура.ПолучитьСсылку(UUID);
			UUID2 				= Новый УникальныйИдентификатор(МассивЗначений[1]);
			ИскХарактеристика	= Справочники.ХарактеристикиНоменклатуры.ПолучитьСсылку(UUID2);
...
Функция УНФ_ВернутьДокументОплаты(Идентификатор, ТипДокументаОплаты)
	...
	Если ТипДокументаОплаты  = "Поступление наличных денежных средств" Тогда
		...
		, Обработки.crm_RetailCRMОбработка.УНФ_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_ПоступлениеВКассу));
		
	ИначеЕсли ТипДокументаОплаты = "Поступление безналичных денежных средств" Тогда
		
		Запрос.УстановитьПараметр("Свойство"
		, Обработки.crm_RetailCRMОбработка.УНФ_ПолучитьСвойство("ИДоплаты", Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_ПоступлениеНаСчет));
...
Функция НайтиДокументыТаблицы(ТаблицаЗаказов)
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	|	ЕСТЬNULL(ДополнительныеСведения.Объект, ЗНАЧЕНИЕ(Документ.ЗаказПокупателя.ПустаяСсылка)) КАК ИскДок,
	|	ДополнительныеСведения.Значение
	|ИЗ
	|	РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
	|ГДЕ
	|	ДополнительныеСведения.Свойство = &Свойство
	|	И ДополнительныеСведения.Значение В (&Значения)");
						
	Запрос.УстановитьПараметр("Значения", ТаблицаЗаказов.ВыгрузитьКолонку("НомерЗаказа"));
	ЗаказПокупателяПредопределенное = Справочники.НаборыДополнительныхРеквизитовИСведений.Документ_ЗаказПокупателя;
	СвойствоЗаказов = Обработки.crm_RetailCRMОбработка.УНФ_ПолучитьСвойство("ИД", ЗаказПокупателяПредопределенное);
	Запрос.УстановитьПараметр("Свойство", СвойствоЗаказов);
	...
	ТаблицаЗаказов.Колонки.Добавить("ИскДок", Новый ОписаниеТипов("ДокументСсылка.ЗаказПокупателя"));
	
	Если Выборка.Количество() <> 0 Тогда
	
		Для каждого Заказ Из Выборка Цикл
			СтрокаТаблицы = ТаблицаЗаказов.Найти(Формат(Заказ.Значение, "ЧГ=0"), "НомерЗаказа");
			СтрокаТаблицы.ИскДок = Заказ.ИскДок;
		КонецЦикла;	
