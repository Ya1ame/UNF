...
Процедура НачатьРаботу(Команда)
		...
		Если Ответ["success"] Тогда 
			Если типзнч(Ответ["sitesAvailable"]) = тип("Массив") Тогда
				...
				Если НайдМагазин = Неопределено Тогда
					ПоказатьОповещениеПользователя(НСтр("ru = 'Для данного Api-Ключа не разрешено использование указанного магазина!'"));
					Возврат;
				КонецЕсли;
			Иначе
				ПоказатьОповещениеПользователя(НСтр("ru = 'Для данного Api-Ключа не разрешено использование указанного магазина!'"));	
	...
	Исключение
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Произошла ошибка при попытке проверки введенных данных, проверьте пожалуйста введенные данные!'"));
		Возврат;
