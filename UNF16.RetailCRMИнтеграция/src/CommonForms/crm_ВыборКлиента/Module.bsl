...
Функция СписокУчастийВПЛ(НомерТел, НомерКарты) Экспорт
	...
	Если Ответ["loyaltyAccounts"].Количество() <> 0 Тогда
		...
		Для каждого Лояльность Из Ответ["loyaltyAccounts"] Цикл
			...
			СтрТЗ.ДатаСоздания 			= crm_RetailCRMОбщийВызовСервера.ПреобразоватьДатуCRM(Лояльность["createdAt"]);
			СтрТЗ.ДатаАктивацииУчастия 	= crm_RetailCRMОбщийВызовСервера.ПреобразоватьДатуCRM(Лояльность["activatedAt"]);
