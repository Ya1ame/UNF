...
Процедура РасчетБаллов(Команда)
	...
	АдресТоваров = ПоместитьТоварыВоВременноеХранилище(ТаблицаТоваров);
	...
	ДоступноДляСписания = Окр(Товары.Итог("Сумма"), 0, РежимОкругления.Окр15как10);
...
Процедура ВывестиДанныеУчастникаПЛ(РезультатОткрытияФормы, ДополнительныеПараметры) Экспорт 
	...
	Если НЕ РезультатОткрытияФормы = Неопределено Тогда 
		...
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, РезультатОткрытияФормы, "Баллы, ФИО, IDКлиента, УчастникПЛ");
		Уровень = РезультатОткрытияФормы.Уровень;
		
		МожноОплатить = Окр(Баллы, 0, РежимОкругления.Окр15как10);
		Если Товары.Количество() Тогда
			МожноОплатить = Окр(Товары.Итог("Сумма"), 0, РежимОкругления.Окр15как10);		
		КонецЕсли;	

		Если МожноОплатить >= Баллы Тогда
			МожноОплатить = Баллы;		
		КонецЕсли;
		
		Если УчастникПЛ И Баллы <> 0 Тогда 
			СтруктураПараметров = Новый Структура;
		...
		Элементы.ИнфоКлиента.Заголовок = 	
								...
								"Количество бонусов: " + Баллы + Символы.ПС +
...
&НаСервере
Функция ПоместитьТоварыВоВременноеХранилище(ТаблицаЗначений)
	
	ТабТоваров = ТаблицаЗначений.Выгрузить();
	АдресТоваров = ПоместитьВоВременноеХранилище(ТабТоваров, УникальныйИдентификатор);
